...
class PatchAnalysisTest(unittest.TestCase):
    ...
    def test_bug_analysis(self):
        ...
        self.assertEqual(info['code_churn_overall'], 190)
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            self.assertEqual(str(w[0].message), 'Revision d0ab0d508a24 was not found.')
            self.assertEqual(str(w[1].message), 'Revision 9f4983dfd881 was not found.')
            self.assertEqual(info['backout_num'], 1)
            self.assertEqual(info['blocks'], 1)
            ...
            self.assertEqual(info['modules_num'], 3)
            self.assertEqual(info['r-ed_patches'], 0)
            self.assertEqual(info['code_churn_overall'], 252)
        ...
        self.assertEqual(info['depends_on'], 29)
        self.assertEqual(info['comments'], 100)
        self.assertEqual(info['changes_size'], 2752)
        self.assertEqual(info['modules_num'], 11)
        self.assertEqual(info['r-ed_patches'], 0)
        self.assertEqual(info['code_churn_overall'], 8200)
        self.assertEqual(info['code_churn_last_3_releases'], 811)
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            self.assertEqual(info['r-ed_patches'], 0)
            self.assertEqual(info['code_churn_overall'], 125)
